...
VIDEO_URLS = [
    "https://www.youtube.com/watch?v=TifRTlamAcs",  # Bob Artemenko
    "https://www.youtube.com/watch?v=6QkQwQ7g1vA",  # Next video (replace with actual 2nd video if different)
]
USER_DATA_DIR = "playwright_user_data_firefox"

async def get_youtube_transcript(video_url, page, idx):
    await page.goto(video_url)
    print(f"[{idx+1}] Navigated to video, waiting 5 seconds...")
    ...
    await page.screenshot(path=f"youtube_page_{idx+1}.png")
    print(f"Screenshot saved as 'youtube_page_{idx+1}.png'.")
    ...
    try:
        ...
        for bidx, btn in enumerate(more_buttons):
            try:
                visible = await btn.is_visible()
                enabled = await btn.is_enabled()
                print(f"Trying 'More' button {bidx}: visible={visible}, enabled={enabled}")
                if visible and enabled:
                    await btn.click()
                    print(f"Clicked 'More' button {bidx}, waiting 3 seconds...")
                    await page.wait_for_timeout(3000)
                    more_clicked = True
                    break
                else:
                    ...
                    if box:
                        ...
                        print(f"Clicked 'More' button {bidx} by coordinates, waiting 3 seconds...")
                        await page.wait_for_timeout(3000)
                        more_clicked = True
                        break
            except Exception as e:
                print(f"Error clicking 'More' button {bidx}: {e}")
    except Exception as e:
    ...
    try:
        ...
        fname = f"youtube_transcript_{idx+1}.txt"
